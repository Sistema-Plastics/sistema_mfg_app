...
const LastPallet = ({ machineID, datasets }) => {
	...
	useEffect(() => {
		datasets.palletdata &&
		datasets.palletdata.jobnum === datasets.currentJob.jn
			? setPltData(datasets.palletdata)
			: setPltData(null);
	}, [datasets.palletdata, datasets.currentJob.jn]);

	useEffect(() => {
		if (debug) {
			if (datasets.palletdata) {
				console.log("palletdata:", datasets.palletdata);
			}
			if (datasets.palletdataconf) {
				console.log("palletdataconf:", datasets.palletdataconf);
			}
		}
		if (
			datasets.palletdataconf &&
			datasets.palletdata &&
			datasets.palletdata.jobnum === datasets.currentJob.jn &&
			datasets.palletdata.tstamp === datasets.palletdataconf.tstamp &&
			datasets.palletdata.jobnum === datasets.palletdataconf.jobnum
		) {
			setPltData((prev) =>
				prev !== datasets.palletdataconf
					? datasets.palletdataconf
					: prev
			);
		}
	}, [datasets.palletdataconf, datasets.palletdata]);
		...
		if (pltData) {
			try {
				...
				switch (pltData.status) {
					case 0:
						tmp.statusText = "Waiting ERP Confirmation";
						tmp.status = 0;
						break;
					case 1:
				...
				if (pltData.status !== 2) {
					...
					const requestedTime = Math.floor(
						parseFloat(pltData.tstamp) / 1000
					);
	...
	useEffect(() => {
		if (debug) console.log("LastPallet.js useEffect fires every time");
	});
