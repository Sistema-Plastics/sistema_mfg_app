...
export default function Content({ machineID, ibdData }) {
	...
	const resGrpDept = ["MACH", "MAUT", "PK Table"];
		...
		try {
			...
			if (jn != null) {
				...
				if (tmpJob != null) {
					...
					const jobTraveller = datasets.jobtraveller.value.filter(
						(job) => job.JobNum === jn
					);
	...
	const handleTreeItemClick = (itemId) => {
		...
		if (!itemId) {
			return;
		} // If the itemId is null, return

		const itemParts = itemId.split("-").map(Number);
		const [, jobAsmblIndex, jobOperIndex] = itemParts;

		const jobAsmbl =
			datasets.currentJob?.jobTraveller?.JobAsmbl[jobAsmblIndex];
		...
		if (!isNaN(jobOperIndex)) {
			...
			if (selectedJobOper && Array.isArray(selectedJobOper)) {
				...
				jobOper = [
					{ AssemblySeq: jobAsmbl.AssemblySeq, ...selectedJobOper },
				];
			} else {
			...
			jobOper =
				jobAsmbl?.JobOper.length > 0
					? jobAsmbl?.JobOper.map((jobOper) => ({
							AssemblySeq: jobAsmbl.AssemblySeq,
							...jobOper,
					  }))
					: [];
		...
		const jobOpDtl = jobOper.reduce((acc, jobOper) => {
			return acc.concat(
				jobOper.JobOpDtl?.map((opDtl) => ({
					OprSeq: jobOper.OprSeq,
					...opDtl,
				})) || []
			);
		}, []);
	...
	const buildTreeData = (job) => {
		const treeData =
			job.JobAsmbl?.map((jobAsmbl, jobAsmblIndex) => {
				const { AssemblySeq, PartNum, Description, JobOper } = jobAsmbl;

				const jobOperations =
					JobOper?.map((jobOper, jobOperIndex) => {
	...
	const jobTravellerTreeData =
		datasets.currentJob?.jobTraveller &&
		Array.isArray(datasets.currentJob.jobTraveller.JobAsmbl)
	...
	const jobOpDtlColumns = [
		...
		{ field: "ResourceID", headerName: "Resource ID", flex: 1 },
		{
			...
			headerName: "Res Description",
			flex: 2,
		},
		{
			field: "SetupOrProd",
			headerName: "Setup/ Prod",
			align: "center",
			headerAlign: "center",
			flex: 1,
		},
		...
		return (
			<React.Fragment>
				...
				<ThemeProvider theme={sistemaTheme}>
					...
					>
						<DialogTitle>
							Job Traveller {datasets.currentJob?.jn}
						</DialogTitle>
						<DialogContent>
	...
	if (!hasJobAssigned) {
		return isMattecMachine
			...
			: displayError(
					`Resource ${machineID} may be a non-Mattec resource. Please start job in Epicor MES`
			  );
	}

	return displayError(
		"Cannot display job details. Please contact IT for assistance."
	);
}
