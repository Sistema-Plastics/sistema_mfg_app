...
const assyShifts = (dataset) => {
  ...
  let tmp = dataset.map((c) => {
    ...
    todayshiftStart.setHours(starttime,(60 * (Math.ceil(starttime) - starttime)), 0 );

    //need to add 24 to ensure end time for night sifts wrap arounfd midnght
    todayshiftEnd.setHours(endtime > starttime ? endtime : endtime + 24,(60 * (Math.ceil(endtime) - endtime)), 0);

    //move the offset 2 hours before for shift planning horizon
    const todayshiftStartOffset = new Date(todayshiftStart);
    const todayshiftEndOffset = new Date(todayshiftEnd);
    todayshiftStartOffset.setHours(todayshiftStart.getHours() - 2);
    todayshiftEndOffset.setHours(todayshiftEnd.getHours() - 2);

    let rv = {
      shiftID: c.Shift,
      shiftDesc: c.Description,
      startTime: todayshiftStart,
      endTime: todayshiftEnd,
      startoffset: todayshiftStartOffset,
      endoffset:todayshiftEndOffset,
