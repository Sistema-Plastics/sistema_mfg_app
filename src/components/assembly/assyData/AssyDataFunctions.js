...
export const AssyData = {
  assyCells: function (dataset) {
    ...
    dataset.length === 0
      ? (tmpAssyCells = [])
      : (tmpAssyCells = dataset
          .filter((c) => c.LongDesc === "ASPK")
          .map((c) => {
            let renamedC = {
              cellID: c.CodeID,
              cellDesc: c.CodeDesc,
  ...
  assyJobs: function (datasets, line, job) {
    ...
    let retLowStock = datasets.jobstockcheck.filter(
      (j) => j.enoughStock == false
    );
    ...
    if (lineParam === undefined && jobParam === undefined) {
      ...
      //     axiosConfigs.config("GET")
      //   );
      getAssyJobs = datasets.jobs.filter(
        (j) => j.JCDept == "ASPK" && j.JobNum == { job }
      );
    } else {
      //   getAssyJobs = await Axios.get(
      ...
      getAssyJobs = datasets.jobs.filter(
        (j) =>
          j.JCDept == "ASPK" && j.JobNum == { job } && j.ResourceID == { line }
      );
    }
    ...
    if (getAssyJobs.length > 0) {
      tempAssyJobs = getAssyJobs.map((e) => {
        ...
        var jobLabourDtl = tempLabourDtl.filter((l) => l.JobNum === e.JobNum);

        var stdPalletQty = Number(e.QtyPerPallet);
        var qtyReceived = Number(e.JobProd_ReceivedQty);

        var fullPalCompCount = Math.floor(e.QtyCompleted / stdPalletQty);
        ...
        var lastFullPalletRcvd = jobLabourDtl.filter(
          (l) => l.JobNum === e.JobNum && l.LaborQty === e.QtyPerPallet_c
        );
        ...
        var setup = tempSetup.filter((s) => s.JobNum === e.obNum);
        ...
        var partBinStockLevel = binStock.filter((s) => s.job === e.JobNum)[0];
        ...
        let renamedE = {
          ...
          constraint: operationsFunctions.stockCheck(e.JobNum, tempLowStock),
          binStockLevel:
          ...
          setupIsComplete: setupComplete.some((l) => l.JobNum === e.JobNum)
            ? "YES"
            : "NO",
          ...
          lastFullQtyBooked:
            lastFullPalletRcvd.length > 0
              ? parseInt(
                  lastFullPalletRcvd[lastFullPalletRcvd.length - 1].LaborQty
                ).toFixed(2)
              : 0,
  ...
  assyShifts: function (dataset) {
    let retVal = null;

    let tmp = dataset.map((c) => {
      let todayshiftStart = new Date();
      let todayshiftEnd = new Date();
      const starttime = parseFloat(c.StartTime);
      const endtime = parseFloat(c.EndTime);
      todayshiftStart.setHours(starttime);
      //need to add 24 to ensure end time for night sifts wrap arounfd midnght
      todayshiftEnd.setHours(endtime > starttime ? endtime : endtime + 24);
