...
import {  AccordionDetails, Box,  Button,  Dialog,  DialogTitle,  FormControl,  FormControlLabel,  Grid,  Typography,  TextField,  Table,  TableRow,  TableCell,  TableBody,  Tooltip,  Link, List,  ListItem,  ListItemAvatar,  Avatar,  ListItemText,} from "@mui/material";
...
import {
    helperfunctions
} from "../../../assets/scripts/helperfunctions";
 ...
 JSON.parse(message.toString())
    ...
    useEffect(() => {
        ...
        if (
            ...
            let jc = datasets.assycrewsizes.filter((a) => a.ResourceID.toUpperCase() === resourceID)[0];
            ...
            if (jc.ResourceGrpID === "") {
                console.warn(`Resource ${resourceID} is not assigned to a Resource Group`);
                return;
            }

            let el = datasets.employees.filter((e) => e.ResourceGrpID === jc.ResourceGrpID);
            let act = datasets.activelabour.filter((e) => e.ResourceID === jc.ResourceID);
            ...
            el = el.filter((e) => datasets.assycrewsizes.filter((ac) => ac.EmployeeNum === e.EmpID).length === 0);
            ...
            if (!jb) {
                console.warn(`Job details for ${jc.JobNum}-${jc.AssemblySeq}-${jc.OprSeq} not in jobsallops`);
                return;
            }
    ...
    const handleLogIn = (resource) => {
        ...
        if (v.length > 0) {v.sort((a, b) => {
                let adate = new Date(a.MattecStartDate);
                let bdate = new Date(b.MattecStartDate);
                adate.setHours(a.MattecStartHour.substring(0, a.MattecStartHour.indexOf(".")));
                bdate.setHours(b.MattecStartHour.substring(0, b.MattecStartHour.indexOf(".")));
                ...
                <Box sx={{ display: "flex", width: "100%", margin: 0 }}>
                  <Typography variant="h6">
                    <Grid container>
                      ...
                      <Grid itemsx={1}>
                        <ColumnHeaderItem>Qty Required</ColumnHeaderItem>
